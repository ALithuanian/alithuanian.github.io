...
Our main source of data is the Spotify API; our secondary data source was the Million Playlists Data. We explored other possible datasets, including Last.fm (as discussed in Milestone 2) and LyricWikia (exploration code seen below), but found these to be insufficient in their scope and depth of coverage to fully capture the connections and details needed for an accurate and effective playlist generation model.   
...
def search_missing_lyrics(df):
    # keep lyrics as is if already found
    if df.lyrics:
        return df.lyrics
